# 8-Way Energy-Aware A* Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Upgrade the Energy-Aware A* algorithm to support 8-way movement with Euclidean distance, proportional turn penalties, and strict corner-cutting prevention.

**Architecture:** Extend the search state to 8 headings. Calculate turn penalties in 45° increments and use √2 (1.414) as the step distance for diagonal moves. Implement a safety check to ensure diagonal moves don't "cut through" corners where walls are present.

**Tech Stack:** TypeScript.

---

### Task 1: Update Heading Type

**Files:**
- Modify: `src/algorithms/astar/astarEnergyAware.ts`

- [ ] **Step 1: Update the Heading type definition**
...
git commit -m "feat: expand Heading type to 8 directions"
```

### Task 2: Update getTurnCost for Proportional Penalties

**Files:**
- Modify: `src/algorithms/astar/astarEnergyAware.ts`

- [ ] **Step 1: Update getTurnCost to use 45° increments**
...
```bash
git add src/algorithms/astar/astarEnergyAware.ts
git commit -m "feat: implement proportional turn penalties for 8-way movement"
```

### Task 3: Update getEnergyCost for Step Distance

**Files:**
- Modify: `src/algorithms/astar/astarEnergyAware.ts`

- [ ] **Step 1: Update getEnergyCost to factor in diagonal distance**
...
```bash
git add src/algorithms/astar/astarEnergyAware.ts
git commit -m "feat: factor in diagonal step distance in energy cost"
```

### Task 4: Implement 8-Way Search with Corner-Cutting Prevention

**Files:**
- Modify: `src/algorithms/astar/astarEnergyAware.ts`

- [ ] **Step 1: Update neighbors and add corner-cutting check**
...
```bash
git add src/algorithms/astar/astarEnergyAware.ts
git commit -m "feat: implement 8-way search with Euclidean heuristic and corner-cutting prevention"
```
